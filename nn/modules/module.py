...
class Module(object):
    ...
    def register_buffer(self, name, tensor):
        ...
        # TODO
        pytorch模型的param都是要训练的，因为param是要反复取出来的，所以要放在buffer里面
        比较方便嘛。那除此之外什么样的tensor也要作为buffer呢？
        比如BatchNorm中的running_mean，它虽然不是模型的参数，但它在训练过程中也是反复要用到的。
        """
    ...
    def _apply(self, fn):
        """
        注意！记得_apply方法是作用于module的param和buffer的
        :param fn:函数
        :return: module自身
        """

        # 下面这个咋一看以为会陷入无限迭代，但仔细一想应该本身模型不是自己的chilren()里面的
        # 经验证果然不是
        ...
        # 不管是要把param修改了，还得对应地修改buffer里面的键值对
    ...
    def cuda(self, device=None):
        ...
        # 按照_apply方法的定义，下面这个 return self._apply(lambda t: t.cuda(device))
        # 是等于把self的param移植到gpu上面， 然后把self的buffer的value移植到gpu上面
